...
import { supportPassive } from '../passive-event';

const eventReg = /^(capture-)?(bind|catch)?([A-Za-z_]+)/;
const { elementPrefix = 'tiny', upperCaseElementPerfix = 'TINY' } = self.tinyRuntimeConfig;

const EVENT_BLACK_LIST= ['click'];
const PRESS_DELAY = 350;
const TAP_DISTANCE = 5;
const TAP_BLACK_LIST = [
  `${upperCaseElementPerfix}-BUTTON`,
  `${upperCaseElementPerfix}-CHECKBOX`,
  `${upperCaseElementPerfix}-RADIO`,
  `${upperCaseElementPerfix}-MAP`,
];

function addListenerToElement(node: Ele, type: string, callback: Function) {
  ...
  if (EVENT_BLACK_LIST.indexOf(eventName) === -1) {
    node.addEventListener(eventName, (e) => {
...
function addTapEvent(node: Ele, options) {
  ...
  node.addEventListener('touchstart', touchstartHandler, supportPassive ? {
    passive: supportPassive,
    capture,
  } : capture);
  node.addEventListener('touchmove', touchmoveHandler, supportPassive ? {
    passive: supportPassive,
    capture,
  } : capture);
