    ...
    var cardApiImpl = {
        ...
        openKBVoucherDetail: {
            ...
            d: function d(opt, cb) {
                callBridge('startApp', {
                    appId: '20000238',
                    param: Object.assign({ target: 'commodity', type: 'market' }, opt)
                }, cb);
            },
    ...
    var api = Object.assign({
        ...
        zmCreditBorrow: {
            d: function d(opt, cb) {
                callBridge('startBizService', {
                    name: 'zm-borrow',
                    param: JSON.stringify(Object.assign({}, opt))
                }, cb);
            }
        }, textRiskIdentification: {}, openCarService: {
            v: function v(opt) {
                if (!('serviceId' in opt)) {
                    return {
                        error: 2,
            ...
            d: function d(opt, cb) {
                function doOpenCarService(addressCode, serviceId) {
                    var url = 'https://mycar-vbizplatformsit.alipay-eco.com/vbizplatform/category/index?cityCode=' + addressCode + '&serviceId=' + serviceId;
                    callBridge('startApp', {
                        appId: '20000919',
                        param: {
                            url: url
            ...
            d: function d(opt, cb) {
                function doOpenLifePayment(addressCode, cityName, instId) {
                    var url = '/www/setNewAccount.htm?subBizType=ELECTRIC&adcode=' + addressCode + '&city=' + cityName + '&instId=' + instId;
                    callBridge('startApp', {
                        appId: '20000193',
                        param: {
                            url: url
            ...
            d: function d(opt, cb) {
                callBridge('startBizService', {
                    name: 'credit-rent',
                    param: JSON.stringify(Object.assign({ creditRentType: 'rent' }, opt))
                }, cb);
            }
        }, zmRentTransition: {
            d: function d(opt, cb) {
                callBridge('startBizService', {
                    ...
                    param: JSON.stringify(Object.assign({ creditRentType: 'signPay' }, opt))
                }, cb);
            }
        }, chooseInvoiceTitle: {
            d: function d(opt, cb) {
