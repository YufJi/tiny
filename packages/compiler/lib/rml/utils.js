...
function getRawJson(obj) {
  ...
  Object.keys(obj).forEach((k) => {
    code.push(`'${startsWith(k, 'data-') || startsWith(k, 'aria-') ? k : camelCase(k)}' : ${obj[k] ? ` ${obj[k]}` : 'true'}`);
  });
  return `{${code.join(',')}}`;
}
...
function getRawJSXAttributeFromJson(transformedAttrs) {
  ...
  Object.keys(transformedAttrs).forEach((k) => {
    code.push(`${startsWith(k, 'data-') || startsWith(k, 'aria-') ? k : camelCase(k)}${transformedAttrs[k] ? ` = ${transformedAttrs[k]}` : ''}`);
  });
  return `${code.join(' ')}`;
}
