let JSTokenizer = require('./JSTokenizer');

if (JSTokenizer.default) {
  JSTokenizer = JSTokenizer.default;
}

const decodeCodePoint = function decodeCodePoint(e) {
  return e;
};
const entityMap = {};
const legacyMap = {};
...
function Tokenizer(options, cbs) {
  ...
  // js插值词法解析。在处于文本、属性值
  this._jsTokenizer = new JSTokenizer((() => {}), ((error) => {
    error._index = _this._index;
    throw error;
  }));
}
...
Tokenizer.prototype._stateText = function (c) {
  // fix #16
  this._jsTokenizer.append(c);
  if (!this._jsTokenizer.stateInJS()) {
    if (c === '<') {
...
Tokenizer.prototype._clearJsTokenizer = function () {
  if (this._state === TEXT || this._state === IN_ATTRIBUTE_VALUE_DQ || this._state === IN_ATTRIBUTE_VALUE_SQ || this._state === IN_ATTRIBUTE_VALUE_NQ) {
    this._jsTokenizer.clear();
  }
};
...
Tokenizer.prototype._parse = function () {
  ...
  if (this._jsTokenizer.stateInJS()) {
    // 解析结束后仍处于js插值解析，提示没有插值结束符号错误
    const error = new Error('parse js error: missing }}');
    error._index = this._index - this._jsTokenizer.getAst().pop().text.length;
    throw error;
  }
